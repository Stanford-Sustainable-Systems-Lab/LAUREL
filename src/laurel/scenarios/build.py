...
- **``task_N`` directory layout**: giving each task its own subdirectory
  ...
  makes it trivial for :class:`~laurel.scenarios.read.ScenarioReader` to
...
- **``results_partition`` injection**: the generated config dict always
  ...
  and ``read_scenario_partition`` in :mod:`laurel.scenarios.io` read this
...
- **``n_tasks_generated`` side-effect**: :meth:`ScenarioBuilder.build_configs`
  sets this attribute so that :class:`~laurel.scenarios.cmd.ScenarioBashWriter`
...
class ScenarioBuilder(ABC):
    ...
    def partition_level_names(self) -> tuple[str]:
        ...
        :class:`~laurel.scenarios.read.ScenarioReader` uses them to extract
